...
class H264StreamEncoder:
    ...
    生命周期:
        ...
        configure(W, H, fps, br)  -> 首次: 创建; 再次: Reconfigure (或必要时重建)
        encode(rgb_hwc)           -> 编码一帧, 追加到当前 bitstream
        finalize(path)            -> 刷尾, 落盘, 清空 bitstream (encoder 保留可复用)
        close()                   -> 释放底层 encoder
    """
    ...
    def configure(
        ...
        """首次创建; 后续若分辨率/编解码器不变则走 Reconfigure, 否则重建."""
        need_recreate = self._encoder is None or W != self._W or H != self._H
